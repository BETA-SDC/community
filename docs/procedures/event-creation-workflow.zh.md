...
- 前缀后空一格，再写活动名称或事项名称
- 当标题确实包含多个字段时，使用 ` - ` 分隔字段，例如任务、通知归档和收尾 Sub-issue；活动主 Issue 通常只写活动名称，不需要额外分隔符
...
- [ ] 海报或宣传物料任务已 assign 给执行人，并已 @ 活动负责人或宣传负责人
- [ ] 海报信息已按模板填写，报名链接和图片使用说明已补全，相关图片或二维码已作为 Sub-issue 附件提交
- [ ] 已从 Issue 创建并使用对应分支
- [ ] 活动材料放在正确目录下
