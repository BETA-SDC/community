...
- 前缀后空一格，再写活动名称或事项名称
- 当标题确实包含多个字段时，使用 ` - ` 分隔字段
- 活动主 Issue 通常只写活动名称，不需要额外分隔符
- 同一活动的活动名称应保持一致
- 涉及日期时统一使用 `YYYY-MM-DD`

> [!TIP]
> `[event] BETA MEET: The Field Experience of an Ecologist` 是活动主 Issue 标题；`[task] BETA MEET: The Field Experience of an Ecologist - 制作海报` 才是多字段任务标题。
